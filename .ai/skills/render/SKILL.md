...
**Archetype log**: read the last 3 lines of `.ai/archetypes.jsonl` (one JSON object per line: `{"slug", "archetype", "palette", "font_pair", "colors": {"bg", "ink", "accent"}}` where `slug` is the full folder name `NN-slug`). Do not reuse any of those three archetypes (or their dominant palette / font pair) for the new render. After picking, append a one-line JSON entry for this story — including the three hex colors that the index card-grid uses as the per-card hover preview. This replaces the older "scan all prior `stories/*/index.html`" rule — that scan is no longer needed.
...
| Number (`412`, `78`, `18 942`) | `<span class="w" data-tr="…" data-pos="числ." data-lemma="…" data-grammar="…" data-si="N">412</span>` — same popup machinery as a word, but the entry is **auto-generated by `render.py`** (see "Number popups" below), not authored in the `.toml` |
...
### Number popups (auto-generated)

Digit runs in the body get the **same** popup as a word, but their entry is computed by `render.py` at tokenize time — `enrich` does **not** need to list them. For each number the script emits:

- **`data-lemma`** — the Spanish cardinal spelling (e.g. `412` → `cuatrocientos doce`, `21` → `veintiuno`, `100` → `cien`). This is the "words representation" shown on the popup's lemma line, and it powers the SpanishDict link.
- **`data-tr`** — the Russian reading (nominative, masculine baseline: `412` → `четыреста двенадцать`).
- **`data-pos`** — always `числ.` (numeral).
- **`data-grammar`** — present only for the classic A1 gotchas, otherwise empty:
  - numbers ending in 1 (`1`, `21`, `121`, …) → the **uno → un / una** apocope + gender forms (`veintiún libro` / `veintiuna casa`)
  - exactly `100` → **cien vs ciento**
  - any number whose hundreds place is 2–9 → the **-cientos / -cientas** gender agreement

Span class is `w` (not a new class), so the existing hover/popup/Shift-to-sentence/detokenize machinery all apply unchanged and re-runs stay idempotent. Numbers get a `data-si` like words, so Shift-to-sentence covers them too.

**Token shape**: a run of digits, optionally with space / NBSP / narrow-NBSP thousands grouping (`18 942` is one token, read as `dieciocho mil novecientos cuarenta y dos`). A trailing `.` after digits (`Día 412.`) stays a sentence-terminator. Designation digits attached by a hyphen (`Anillo-7`, `Pioneer-12`) are wrapped too and read as plain cardinals (`siete`, `doce`).

**Override**: to customize a specific token (a code that should not read as a cardinal, a custom gloss), add a `[numbers."<digits>"]` table in `enrichment.toml`; its fields are merged over the auto-generated entry. Key by the exact surface form including any grouping space. See the enrich skill for the format.
...
4. **Pick the format archetype** from the named list (or invent a fresh one). Read the last 3 lines of `.ai/archetypes.jsonl` and do not reuse any of those archetypes (or their dominant palette / font pair). After deciding, append a one-line JSON entry `{"slug":"NN-slug","archetype":"…","palette":"…","font_pair":"…","colors":{"bg":"#hex","ink":"#hex","accent":"#hex"}}` to `.ai/archetypes.jsonl`. The three hex colors should be the page's dominant background, primary text, and accent — they power the per-card hover preview on the project-wide index.
