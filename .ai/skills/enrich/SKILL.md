...
### Numbers — auto-handled, override only when needed

You do **not** enrich numbers. Digit runs in the body (`412`, `78`, `Día 29`, `18 942`, `Anillo-7`) get their popup generated by `render.py` at render time: the Spanish cardinal spelling (`cuatrocientos doce`) as the lemma, a Russian reading (`четыреста двенадцать`) as `tr`, `pos = "числ."`, and an auto grammar note for the A1 gotchas (uno→un/una apocope, cien/ciento, -cientos/-cientas agreement). No `.toml` entry is required and none of these count as "missed words".

Add a `[numbers."<digits>"]` table **only** to override a specific token — e.g. a designation that should not read as a cardinal, or a number that needs a custom gloss. Key the table by the **exact surface form** (including any grouping space); the fields you set are merged over (and win against) the auto-generated entry:

```toml
# read "2031" as a year, not the bare cardinal
[numbers."2031"]
tr = "2031 год (год отправки зонда)"
grammar = """
**dos mil treinta y uno** — год читается как обычное число.
"""

# a designation code that should keep a literal gloss
[numbers."7"]
tr = "седьмой (номер станции Anillo-7)"
```

Any field you omit keeps its auto-generated value. Leave numbers alone unless an override genuinely helps the reader.
